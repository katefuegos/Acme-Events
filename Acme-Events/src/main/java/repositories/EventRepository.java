...
public interface EventRepository extends JpaRepository<Event, Integer> {
	...
	@Query("select e from Event e where e.status <> 'CANCELLED' and e.momentEnd>=current_date and e.club.id in (select c.id from Club c join c.follows f where f.client.id = ?1)")
	Collection<Event> findAvailableEventsByFollower(int clienId);

	@Query("select e from Event e where e.status <> 'CANCELLED' and e.momentEnd<current_date and e.club.id in (select c.id from Club c join c.follows f where f.client.id = ?1)")
	Collection<Event> findFinishedEventsByFollower(int clienId);

	@Query("select e from Event e where e.status = 'CANCELLED' and e.club.id in (select c.id from Club c join c.follows f where f.client.id = ?1)")
	...
	@Query("select e from Event e where e.status <> 'CANCELLED' and e.momentEnd>=current_date and e.club.id in (select c.id from Club c join c.follows f where f.client.id = ?1 and c.id=?2)")
	...
	@Query("select e from Event e where e.status = 'CANCELLED' and e.club.id in (select c.id from Club c join c.follows f where f.client.id = ?1 and c.id=?2)")
