...
public interface AdministratorRepository extends JpaRepository<Administrator, Integer> {
	...
	@Query("select count(e)*1.0 from Event e where e.draftMode=false and e.status = 'AVAILABLE'")
	Double queryC5();

	//C6 - The ratio of cancelled events.
	@Query("select count(e)*1.0 from Event e where e.draftMode=false and e.status = 'CANCELLED'")
	Double queryC6();
