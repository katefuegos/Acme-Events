...
select avg(1.0 * (select count(e) from Event e where e.club.id = b.id)),min(1.0 * (select count(e) from Event e where e.club.id = b.id)),max(1.0 * (select count(e) from Event e where e.club.id = b.id)),stddev(1.0 * (select count(e) from Event e where e.club.id = b.id)) from Club b


select avg(e.participationsEvent),min(e.participationsEvent),max(e.participationsEvent),stddev(e.participationsEvent) from Event e;
